...
export default function CadastroMobiliario() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await push(refMobiliario, form)
      showNotification("Mobiliário cadastrado com sucesso!", "success")
      setForm({
        patrimonio: "",
        dataCadastro: "",
        descricao: "",
  ...
  const basicFields = [
    { label: "Patrimônio", name: "patrimonio", placeholder: "Ex: MOB001", required: true },
    { label: "Descrição", name: "descricao", placeholder: "Ex: Mesa de escritório, Cadeira giratória", required: true },
    ...
    { label: "Quantidade", name: "quantidade", placeholder: "Ex: 1, 2, 5", required: true },
    { label: "Ambiente Atual", name: "ambienteAtual", placeholder: "Ex: Sala 101, Recepção", required: false },
  ]
  ...
  const additionalFields = [
    ...
    {
      label: "Projeto-Edital-Convenio",
      name: "projetoEditalConvenio",
      placeholder: "Informações do convênio",
      required: true,
    },
  ]
        ...
        >
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-8 text-white">
            ...
            >
              ...
              <div>
                <h1 className="text-3xl font-bold">Cadastro de Mobiliário</h1>
                <p className="text-indigo-100 mt-1">Registre móveis e equipamentos de ambiente</p>
              </div>
            </motion.div>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {additionalFields.map((field, index) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.1 * index }}
                      className={field.name === "projetoEditalConvenio" ? "md:col-span-2" : ""}
                    >
                      <label className="block text-sm font-semibold text-gray-700 mb-2">
                        {field.label}
                        {field.required && <span className="text-red-500 ml-1">*</span>}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      value={form.obs}
                      onChange={handleChange}
                      placeholder="Informações adicionais sobre o mobiliário..."
                      disabled={isSubmitting}
                      rows={3}
                      className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-gray-700 
                               focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 
