...
export default function Home() {
  ...
  // Controle do contador de chamados novos para o badge
  const [itSupportCount, setItSupportCount] = useState(0)
  const chamadosConhecidosRef = useRef(new Set()) // track chamados já notificados
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(chamadosRef, (snapshot) => {
      const data = snapshot.val()
      if (data) {
        let novosChamadosCount = 0

        Object.entries(data).forEach(([uid, chamadosUsuario]) => {
          Object.entries(chamadosUsuario).forEach(([id, chamado]) => {
            const chave = `${uid}_${id}`
            if (!chamadosConhecidosRef.current.has(chave)) {
              chamadosConhecidosRef.current.add(chave)
              novosChamadosCount++
              toast.info(`Novo chamado: ${chamado.titulo || id}`, {
                position: "top-right",
                autoClose: 5000,
              })
            }
        ...
        if (novosChamadosCount > 0) {
          setItSupportCount((prev) => prev + novosChamadosCount)
        }
      }
    })
  ...
  const cards = [
    ...
    },
    {
      ...
      onClick: () => {
        navigate("/list-chamados")
        setItSupportCount(0)
        chamadosConhecidosRef.current.clear()
      },
  ...
  // Atualiza o badge do card "IT Support" para exibir contador se > 0
  const cardsComBadgeAtualizado = cards.map((card) => {
    if (card.title === "IT Support") {
      return {
        ...card,
        badge: itSupportCount > 0 ? `+${itSupportCount}` : card.badge,
      }
    }
    return card
  })
          ...
          <div className="max-w-7xl mx-auto">
                        ...
                        <div className="absolute top-3 right-3">
                          <div
                            className={`w-2 h-2 rounded-full ${
                              stat.status === "operational" ? "bg-emerald-400" : "bg-amber-400"
                            } animate-pulse`}
                          />
                        </div>
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {cardsComBadgeAtualizado.map((card, idx) => {
