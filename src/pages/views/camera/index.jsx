...
export default function VisualizarCamera() {
  ...
  const exportarParaExcel = () => {
    if (!tablets.length) {
      toast.warning("Nenhuma câmera para exportar.")
      return
    }

    const data = tablets.map((t) => ({
      ...
      Projeto: t.projeto || "—",
      "Nota Fiscal": t.notaFiscal || "—",
      NCM: t.NCM || "—",
      VR_BEM: t.vrbem || "—",
      Ano: t.ano || "—",
      "Data Cadastro": t.dataCadastro || "—",
      Parceiro: t.parceiro || "—",
      "Projeto/Edital/Convênio": t.projetoEditalConvenio || "—",
      Motivo: t.motivo || "—",
      Observações: t.obs || "—",
    }))
    ...
    XLSX.writeFile(workbook, "cameras_exportadas.xlsx")
  }
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-6">
          <div className="flex items-center gap-3">
            ...
            <motion.button
              ...
              className="ml-auto bg-white text-indigo-600 hover:text-white hover:bg-indigo-700 font-semibold py-2 px-4 rounded-lg text-sm shadow-md transition-all duration-200"
            >
              📥 Exportar Excel
            </motion.button>
          </div>
                    ...
                    <div className="flex gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="space-y-1 text-sm text-gray-600">
                          <p>
                            <span className="font-medium">Câmera:</span> {tablet.camera}
                          </p>
                          <p>
                ...
                <div className="flex gap-6 mb-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-500 block">Câmera:</span>
                        <span className="font-medium text-gray-800">{modalTablet.camera}</span>
                      </div>
                      <div>
...
// export default function ImportarCameras() {
...
//           const camera = {
//             patrimonio,
//             camera: item["camera"]?.toString().trim() || "",
//             marca: item["marca"]?.toString().trim() || "",
//             modelo: item["modelo"]?.toString().trim() || "",
//             local: item["local"]?.toString().trim() || "",
//             status: item["status"]?.toString().trim() || "Disponível",
//             responsavel: item["responsável"]?.toString().trim() || "",
//             projeto: item["projeto"]?.toString().trim() || "",
//             notaFiscal: item["notaFiscal"]?.toString().trim() || "",
//             NCM: item["NCM"]?.toString().trim() || "",
//             vrbem: item["vrbem"]?.toString().trim() || "",
//             ano: item["ano"]?.toString().trim() || "",
//             dataCadastro: item["dataCadastro"]?.toString().trim() || "",
//             parceiro: item["parceiro"]?.toString().trim() || "",
//             projetoEditalConvenio: item["projetoEditalConvenio"]?.toString().trim() || "",
//             motivo: item["motivo"]?.toString().trim() || "",
//             obs: item["obs"]?.toString().trim() || "",
//           }

//           const produtoRef = ref(db, `cameras/${patrimonio}`)
//           return set(produtoRef, camera)
//         })

//         const validPromises = promises.filter(Boolean)
//         await Promise.all(validPromises)

//         toast.success(`✅ ${validPromises.length} câmeras importadas com sucesso!`)
//       } catch (err) {
...
//         📥 Importar planilha de câmeras (.xlsx ou .xls)
//       </label>
//       <Input
//         type="file"
//         accept=".xlsx,.xls"
...
//       />
//       {loading && (
//         <p className="text-blue-700 mt-2">⏳ Importando dados para o Firebase...</p>
//       )}
//     </div>
//   )
// }
