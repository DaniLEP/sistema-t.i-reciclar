...
export default function VisualizarTablets() {
  ...
  const updateTabletData = async (tabletId, statusToSave, motivoToSave = "") => {
    ...
    try {
      await update(tabletRef, {
        status: statusToSave,
        motivo: motivoToSave,
      })
      toast.success("Tablet atualizado com sucesso!", {
        icon: "🎉",
        style: {
      ...
      toast.error("Erro ao atualizar tablet: " + error.message)
    }
  }
  ...
  const handleSalvarStatus = () => {
    ...
    if (
      ["quebrado", "emprestado", "manutencao", "naoencontrado", "controlador", "colaborador"].includes(statusNormalized)
    ) {
  ...
  const exportToExcel = () => {
    ...
    const worksheetData = filteredAndSortedTablets.map(({ patrimonio, marca, modelo, local, status, motivo }) => ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(tabletsRef, (snapshot) => {
      ...
      if (data) {
        ...
        totalCount.set(tabletsArray.length)
      } else {
        setTablets([])
        totalCount.set(0)
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-4 md:p-6">
      ...
      >
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                {tablets.length > 0 && (
                  ...
                  >
                    ...
                    Exportar ({filteredAndSortedTablets.length})
                  </motion.button>
                )}
              </div>
            </div>
              ...
              return (
                <motion.div
                  ...
                  className={`relative p-5 rounded-2xl border-2 cursor-pointer transition-all duration-300 ${
                    isActive
                      ? `${config.color} shadow-lg ring-2 ring-offset-2 ring-indigo-500`
                      : `${config.color} hover:shadow-lg`
                  }`}
        ...
        {/* Enhanced Controls */}
        <motion.div
          className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 p-6 mb-8"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Buscar por patrimônio, marca, modelo ou local..."
                value={searchTerm}
              ...
              <motion.button
                ...
                className={`p-4 rounded-2xl transition-all duration-300 ${
                  viewMode === "grid"
                    ...
                    : "border-2 border-gray-200 hover:bg-gray-50 bg-white/50 backdrop-blur-sm"
                }`}
              >
        ...
        {filteredAndSortedTablets.length === 0 ? (
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-4">
              {tablets.length === 0 ? "Nenhum tablet cadastrado" : "Nenhum tablet encontrado"}
            </h3>
            <p className="text-gray-600 text-lg max-w-md mx-auto">
              {tablets.length === 0
                ? "Comece cadastrando seu primeiro tablet para começar a gerenciar seu inventário"
            ...
            {(searchTerm || filterStatus !== "Todos") && (
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="mt-6 px-6 py-3 bg-indigo-500 hover:bg-indigo-600 text-white rounded-2xl font-semibold transition-all duration-300"
              >
                Limpar Filtros
              </motion.button>
            )}
                  ...
                  >
                    <div className={viewMode === "grid" ? "p-6" : "flex items-center gap-6 flex-1"}>
                      <div className={`flex gap-4 ${viewMode === "grid" ? "mb-6" : ""}`}>
                        <div
                          className={`flex-shrink-0 bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl overflow-hidden flex items-center justify-center relative group-hover:scale-105 transition-transform duration-300 ${
                        ...
                        >
                          {tablet.fotoBase64 ? (
                            <img
                              ...
                              alt={`Tablet ${tablet.patrimonio}`}
                              className="object-cover w-full h-full"
                            />
                          ) : (
                            <TabletSmartphone
                              className={`text-gray-400 ${viewMode === "grid" ? "w-10 h-10" : "w-8 h-8"}`}
                            />
                          )}
                        ...
                        <div
                          className={`flex-1 min-w-0 ${viewMode === "list" ? "flex items-center justify-between" : ""}`}
                        >
                          <div className={viewMode === "list" ? "flex-1" : ""}>
                            <div className="flex items-start justify-between mb-3">
                              <h3
                                className={`font-bold text-gray-900 truncate ${viewMode === "grid" ? "text-xl" : "text-lg"}`}
                              >
                                {tablet.patrimonio}
                              </h3>
                              <motion.span
                                whileHover={{ scale: 1.05 }}
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border-2 shadow-sm ${config.color}`}
                              >
            ...
            >
              <div className="p-8">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    <motion.div
                      className="p-3 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl"
                      whileHover={{ rotate: 5, scale: 1.1 }}
                    >
                      <TabletSmartphone className="w-8 h-8 text-white" />
                    </motion.div>
                ...
                >
                  <div className="grid md:grid-cols-2 gap-6 mb-8">
                    <div>
                      ...
                      <div
                        className={`inline-flex items-center gap-2 px-4 py-3 rounded-2xl text-sm font-semibold border-2 ${STATUS_CONFIG[modalTablet.status].color}`}
                      >
                        {React.createElement(STATUS_CONFIG[modalTablet.status].icon, { className: "w-4 h-4" })}
                        {modalTablet.status}
                      </div>
                    </div>
            ...
            >
              <div className="p-8">
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="w-full mt-6 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-bold py-4 px-8 rounded-2xl transition-all duration-300 shadow-lg hover:shadow-xl"
                >
                  Salvar
                </motion.button>
              </div>
...
//       />
//       {loading && (
//         <p className="text-blue-700 mt-2">⏳ Importando dados...</p>
//       )}
//     </div>
//   );
// }
