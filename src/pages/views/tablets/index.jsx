...
//           modalTablet.status === "Quebrado"
//             ? "Descreva o problema"
//             : "Digite o nome do responsável pelo empréstimo"
//         }
//       />
...
export default function VisualizarTablets() {
  ...
  const handleSalvarStatus = () => {
    ...
    if (["Quebrado", "Emprestado", "Manutencao", "Nao Encontrado"].includes(modalTablet.status)) {
      ...
      {modalTablet && (
        <motion.div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModalTablet(null)}>
          <motion.div className="bg-white rounded-lg max-w-lg w-full p-6 relative" initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4 flex gap-4">
              <div className="flex-shrink-0 w-32 h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                {modalTablet.fotoBase64 ? (<img src={modalTablet.fotoBase64} alt="Foto" className="object-cover w-full h-full" />) : (<TabletSmartphone className="w-12 h-12 text-gray-400" />)}
              </div>
