...
export default function VisualizarTablets() {
  ...
  const handleSalvarStatus = () => {
    ...
    // Corrigi os status para combinar com os que você usa no select e na condição
    const statusNormalized = modalTablet.status.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    if (["quebrado", "emprestado", "manutencao", "naoencontrado"].includes(statusNormalized)) {
    ...
    <div className="min-h-screen bg-gradient-to-r from-purple-600 via-indigo-700 to-gray-900 p-6 flex flex-col items-center">
      ...
      >
        ...
        {tablets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {tablets.map((tablet) => (
              <div
                key={tablet.id}
                className="border border-gray-300 rounded-lg p-4 shadow-sm flex gap-4"
              >
                <div className="flex-shrink-0 w-32 h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                  {tablet.fotoBase64 ? (
                    <img
                      src={tablet.fotoBase64}
                      alt={`Foto do tablet ${tablet.patrimonio}`}
                ...
                <div className="flex flex-col justify-between flex-grow">
                  <div>
                    <p>
                      <strong>Patrimônio:</strong> {tablet.patrimonio}
                    </p>
                    <p>
                      <strong>Marca:</strong> {tablet.marca}
                    </p>
                    <p>
                      <strong>Local:</strong> {tablet.local}
                    </p>
                    <p>
                      <strong>Modelo:</strong> {tablet.modelo}
                    </p>
                    <p>
                      <strong>Status:</strong>{" "}
                      <span
                        className={`font-semibold ${
                          tablet.status === "Disponível"
                            ? "text-green-600"
                            : tablet.status === "Quebrado"
                            ? "text-red-600"
                            : "text-yellow-600"
                        }`}
                      >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          className="mt-8 w-full md:w-auto bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg text-sm shadow-md flex items-center justify-center gap-2"
        >
        ...
        >
          ...
          >
            ...
            <div className="mb-4 flex gap-4">
              <div className="flex-shrink-0 w-32 h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                {modalTablet.fotoBase64 ? (
                  <img src={modalTablet.fotoBase64} alt="Foto" className="object-cover w-full h-full" />
                ) : (
                  <TabletSmartphone className="w-12 h-12 text-gray-400" />
                )}
              </div>
              <div className="text-sm">
                <p>
                  <strong>Patrimônio:</strong> {modalTablet.patrimonio}
                </p>
                <p>
                  <strong>Marca:</strong> {modalTablet.marca}
                </p>
                <p>
                  <strong>Local:</strong> {modalTablet.local}
                </p>
                <p>
                  <strong>Modelo:</strong> {modalTablet.modelo}
                </p>
                <p>
                  <strong>Status atual:</strong> {modalTablet.status}
                </p>
            ...
            <button
              ...
              className="mt-4 bg-indigo-600 hover:bg-indigo-700 text-white w-full py-2 rounded-lg font-semibold"
            >
              Salvar Status
            </button>
          </motion.div>
        ...
        >
          ...
          >
            ...
            <textarea
              ...
              placeholder="Motivo ou responsável"
            />
            <button
