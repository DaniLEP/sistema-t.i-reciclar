...
import { TabletSmartphone, ArrowLeft, X } from "lucide-react";
...
export default function VisualizarTablets() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(tabletsRef, (snapshot) => {
      ...
      if (data) {
        const tabletsArray = Object.entries(data).map(([id, tablet]) => ({id, ...tablet, status: tablet.status || "Disponível", motivo: tablet.motivo || "", }));
  ...
  const statusCount = STATUS_OPTIONS.reduce((acc, status) => {acc[status] = tablets.filter((t) => t.status === status).length; return acc; }, {});
  ...
  const handleSalvarStatus = () => {
    ...
    else {updateTabletData(modalTablet.id, "Disponível", "");}
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-purple-600 via-indigo-700 to-gray-900 p-6 flex flex-col items-center">
      ...
      <motion.div
        className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl p-8" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <div className="flex items-center gap-3 mb-6">
          <TabletSmartphone className="w-7 h-7 text-indigo-600" />
          <h2 className="text-3xl font-bold text-gray-800">Tablets Cadastrados</h2>
        </div>
            ...
            {tablets.map((tablet) => (
              <div key={tablet.id} className="border border-gray-300 rounded-lg p-4 shadow-sm flex gap-4">
                <div className="flex-shrink-0 w-32 h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                  {tablet.fotoBase64 ? (<img src={tablet.fotoBase64} alt={`Foto do tablet ${tablet.patrimonio}`} className="object-cover w-full h-full"/>) : (
                ...
                <div className="flex flex-col justify-between flex-grow">
                  ...
                  <button onClick={() => setModalTablet(tablet)}
                    className="mt-3 self-start px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold transition">Ver mais</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <motion.button whileTap={{ scale: 0.97 }} onClick={() => navigate("/register-option")}
        ...
        <motion.div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div className="bg-white rounded-lg max-w-lg w-full p-6 relative"
            ...
            <div className="mb-4 flex gap-4">
              <div className="flex-shrink-0 w-32 h-24 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                {modalTablet.fotoBase64 ? (<img src={modalTablet.fotoBase64} alt={`Foto do tablet ${modalTablet.patrimonio}`} className="object-cover w-full h-full"/>) : (
                  <TabletSmartphone className="w-12 h-12 text-gray-400" />)}
              </div>
              <div>
            ...
            <button onClick={handleSalvarStatus} className="mt-6 w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-semibold transition"> Salvar Status</button>
          </motion.div>
        </motion.div>
      )}
      ...
      {modalMotivoOpen && (
        <motion.div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModalMotivoOpen(false)}>
          <motion.div className="bg-white rounded-lg max-w-md w-full p-6 relative" initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-3 right-3 text-gray-600 hover:text-gray-900"
              onClick={() => setModalMotivoOpen(false)} aria-label="Fechar"><X className="w-6 h-6" /></button>
            <h3 className="text-xl font-bold mb-4">Informe o motivo ou responsável </h3>
            <textarea value={motivo} onChange={(e) => setMotivo(e.target.value)} disabled={motivoReadonly} rows={5} placeholder="Motivo ou responsável"
              className="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"></textarea>
            {!motivoReadonly && (<button onClick={handleSalvarMotivo}
                className="mt-4 w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-semibold transition"> Salvar Motivo</button>)}
          </motion.div>
        </motion.div>
      )}
    </div>
