            ...
            <tbody>
              {notebooksFiltrados.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-2 border text-center max-w-[250px] truncate" title={item.observacoes || ""}>
                    {item.observacoes || "-"}
                  </td>
                  <td className="p-2 border text-center">
                  ...
                  <td className="p-2 border text-center">
                    <button onClick={() => abrirModal(item)} className="bg-indigo-600 text-white px-3 py-1 rounded hover:bg-indigo-700 transition">
                      Ver Mais
                    </button>
                  </td>
                </tr>
              ...
              >
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-shrink-0 w-full md:w-56 h-40 md:h-56 rounded-lg overflow-hidden border border-gray-300 shadow-sm mx-auto md:mx-0">
                    {selecionado.fotoBase64 ? (
                      <img
                        ...
                        alt={`Foto do notebook ${selecionado.modelo || selecionado.patrimonio}`}
                        className="w-full h-full object-contain bg-gray-50"
                      />
                    ) : (
                      <div className="flex items-center justify-center w-full h-full bg-gray-100 text-gray-400 italic">
                        Sem foto
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6 text-gray-700 text-sm">
                    ...
                    {selecionado.motivo && (
                      <p className="sm:col-span-2"><strong>Motivo:</strong> {selecionado.motivo}</p>
                    )}
      ...
      <AnimatePresence>
        {modalMotivo && (
          <motion.div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
            ...
            <motion.div className="bg-white p-6 rounded-xl shadow-xl max-w-md w-full relative"
              initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }}>
              <button className="absolute top-3 right-3 text-gray-500 hover:text-black" onClick={fecharMotivo}>
                <X />
              </button>
              <h3 className="text-xl font-bold mb-4 text-red-700">Informe o motivo da alteração</h3>
              <textarea className="w-full h-28 p-2 border rounded focus:ring-2 focus:ring-red-500 resize-none"
                value={motivo} onChange={(e) => setMotivo(e.target.value)} placeholder="Digite o motivo..." />
