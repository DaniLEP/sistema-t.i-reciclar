...
export default function VisualizarNotebooks() {
  ...
  const salvarEdicao = async () => {
    ...
    try {
      const db = getDatabase(app)
      await update(ref(db, `notebooks/${selecionado.id}`), {
        ...dadosEdicao,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-4 sm:p-6">
      <motion.div
        className="max-w-7xl mx-auto bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 sm:p-8 overflow-hidden"
      ...
      >
        ...
        >
          ...
          <Button
            ...
            className="flex items-center gap-2 hover:bg-gray-50 border-gray-200 bg-transparent"
          >
        ...
        >
          ...
          <div className="relative">
            ...
            <Select value={filtroStatus} onValueChange={setFiltroStatus}>
              <SelectTrigger className="pl-10 border-gray-200 focus:border-blue-500">
            ...
            >
              <Card className="text-center hover:shadow-md transition-all duration-300 border-0 shadow-sm bg-gradient-to-br from-white to-gray-50">
                <CardContent className="p-4">
                  <div className={`inline-flex px-2 py-1 rounded-full text-xs font-medium mb-2 ${getStatusColor(key)}`}>
                    ...
                    {currentItems.map((item, index) => (
                      ...
                      >
                        ...
                        <TableCell className="text-center">
                          <Badge
                            className={`text-xs font-medium border ${getStatusColor(item.status)}`}
                            title={item.motivo || ""}
                          >
                            {item.status}
                          </Badge>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-2xl font-bold">
                          {modoEdicao ? "Editando Notebook" : "Detalhes do Notebook"}
                        </h2>
                        <p className="text-blue-100 text-sm">
                          {selecionado.modelo || "Modelo não informado"} • {selecionado.marca || "Marca não informada"}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center gap-2">
                      <Badge className={`text-sm font-medium border-2 ${getStatusColor(selecionado.status)} bg-white`}>
                        {selecionado.status}
                      </Badge>
                      {!modoEdicao && (
                        <Button
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Card className="p-4">
                          ...
                          <div className="space-y-4">
                            <div>
                              <Label htmlFor="patrimonio" className="text-sm font-medium text-gray-600">
                                Patrimônio *
                              </Label>
                              <Input
                                id="patrimonio"
                                value={dadosEdicao.patrimonio || ""}
                            ...
                            <div>
                              <Label htmlFor="notebook" className="text-sm font-medium text-gray-600">
                                Notebook
                              </Label>
                              <Input
                                id="notebook"
                                value={dadosEdicao.notebook || ""}
                                onChange={(e) => handleInputChange("notebook", e.target.value)}
                                className="mt-1"
                                placeholder="Nome/Identificação do notebook"
                              />
                            </div>
                            <div>
                            ...
                            <div>
                              <Label htmlFor="modelo" className="text-sm font-medium text-gray-600">
                                Modelo *
                              </Label>
                              <Input
                                id="modelo"
                        ...
                        <Card className="p-4">
                          ...
                          <div className="space-y-4">
                            <div>
                              <Label htmlFor="local" className="text-sm font-medium text-gray-600">
                                Local
                              </Label>
                              <Input
                                id="local"
                            ...
                            <div>
                              <Label htmlFor="projeto" className="text-sm font-medium text-gray-600">
                                Projeto
                              </Label>
                              <Input
                                id="projeto"
                                value={dadosEdicao.projeto || ""}
                            ...
                            <div>
                              <Label htmlFor="parceiro" className="text-sm font-medium text-gray-600">
                                Parceiro
                              </Label>
                              <Input
                                id="parceiro"
                                value={dadosEdicao.parceiro || ""}
                                onChange={(e) => handleInputChange("parceiro", e.target.value)}
                                className="mt-1"
                                placeholder="Parceiro responsável"
                              />
                            </div>
                          </div>
                        </Card>
                        ...
                        <Card className="p-4">
                          ...
                          <div className="space-y-4">
                            <div>
                              <Label htmlFor="notaFiscal" className="text-sm font-medium text-gray-600">
                                Nota Fiscal
                              </Label>
                              <Input
                                id="notaFiscal"
                                value={dadosEdicao.notaFiscal || ""}
                            ...
                            <div>
                              <Label htmlFor="vrbem" className="text-sm font-medium text-gray-600">
                                VR-BEM
                              </Label>
                              <Input
                                id="vrbem"
                                value={dadosEdicao.vrbem || ""}
                                onChange={(e) => handleInputChange("vrbem", e.target.value)}
                                className="mt-1"
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-gray-900 mt-1 whitespace-pre-wrap">
                                {selecionado.obs || "Nenhuma observação registrada."}
                              </p>
                            </div>
                          </div>
                        </Card>
                        ...
                        <Select value={selecionado.status} onValueChange={alterarStatus}>
                          ...
                          <SelectContent>
                            {STATUS_OPTIONS.map((opt) => (
                              <SelectItem key={opt.value} value={opt.value}>
                                <div className="flex items-center gap-2">
                                  <div className={`w-2 h-2 rounded-full ${getStatusColor(opt.value).split(" ")[0]}`} />
                                  {opt.label}
                                </div>
                              </SelectItem>
                            ))}
                ...
                <div className="border-t bg-gray-50 p-6">
                  <div className="flex justify-between items-center">
                    <div className="text-sm text-gray-500">
                      {selecionado.dataUltimaEdicao && (
                        <span>Última edição: {formatDate(selecionado.dataUltimaEdicao)}</span>
                      )}
                    </div>
            ...
            <DialogContent className="max-w-md p-0 rounded-2xl shadow-2xl border-0">
              ...
              >
                <DialogHeader className="mb-4">
                  <DialogTitle className="text-xl font-bold text-red-700 mb-2">
                  ...
                  <DialogClose asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-full"
                    >
                      <X className="w-5 h-5" />
                    </Button>
                  </DialogClose>
